				...
				public void run() {
			        ...
			        for (int i = 0; i < bricks.size(); i++)
					{
			        	if (rightTopX >= bricks.get(i).getLeft() && rightTopX <= bricks.get(i).getRight() && rightTopY >= bricks.get(i).getTop() && rightTopY <= bricks.get(i).getBottom())
			        	{
			        		...
			        		mainBall.setDX(-4);
			        		mainBall.setDY(4);
			        		Log.d("rightTopX", Float.toString(mainBall.getDX()));
			        	}
			        	else if (leftTopX >= bricks.get(i).getLeft() && leftTopX <= bricks.get(i).getRight() && leftTopY >= bricks.get(i).getTop() && leftTopY <= bricks.get(i).getBottom())
			        	{
			        		...
			        		mainBall.setDX(4);
			        		mainBall.setDY(4);
			        		Log.d("LeftTopX", Float.toString(mainBall.getDX()));
			        	}
			        	else if (leftBottomX >= bricks.get(i).getLeft() && leftBottomX <= bricks.get(i).getRight() && leftBottomY >= bricks.get(i).getTop() && leftBottomY <= bricks.get(i).getBottom())
			        	{
			        		...
			        		mainBall.setDX(4);
			        		mainBall.setDY(-4);
			        		Log.d("LeftBottomX", Float.toString(mainBall.getDX()));
			        	}
			        	else if (rightBottomX >= bricks.get(i).getLeft() && rightBottomX <= bricks.get(i).getRight() && rightBottomY >= bricks.get(i).getTop() && rightBottomY <= bricks.get(i).getBottom())
			        	{
			        		...
			        		mainBall.setDX(-4);
			        		mainBall.setDY(-4);
			        		Log.d("RightBottomX", Float.toString(mainBall.getDX()));
			        	}
			        	else if (mainBall.getY() - mainBall.getR() <= bricks.get(i).getBottom() && mainBall.getY() + mainBall.getR() >=bricks.get(i).getTop() && mainBall.getX() >= bricks.get(i).getLeft() && mainBall.getX() <= bricks.get(i).getRight())
						{
							CheckPowerHiddenInBrick (i);
							mainBall.setDY(-mainBall.getDY());
							Log.d("Just Y", Float.toString(mainBall.getDY()));
						}
						else if (mainBall.getX() - mainBall.getR() <= bricks.get(i).getRight() && mainBall.getX() + mainBall.getR() >= bricks.get(i).getLeft() && mainBall.getY() >= bricks.get(i).getTop() && mainBall.getY() <= bricks.get(i).getBottom())
						{
							CheckPowerHiddenInBrick (i);
							mainBall.setDX(-mainBall.getDX());
							Log.d("Just X", Float.toString(mainBall.getDY()));
						}
						else {
			        ...
			        /*
			        if (mainBall.getY() - mainBall.getR() <= bar.getBottom() && mainBall.getY() + mainBall.getR() >= bar.getTop() && mainBall.getX() >= bar.getLeft() && mainBall.getX() <= bar.getRight())
					{
			        	mainBall.setDY(-mainBall.getDY());
					}*/
    	...
    	public void run() {
    		powerBallHandler.post(new Runnable() {
				...
				public void run() {
					...
					if (powerBallTaken && System.currentTimeMillis() >= powerUpTime + 10000)
					{
						mainBall.setR(25);
						powerBallTaken = false;
					}
					
					if (weakBallTaken && System.currentTimeMillis() >= weakeenUpTime + 10000)
					{
						mainBall.setR(25);
						weakBallTaken = false;
					}
					
					if (biggerBarBallTaken && System.currentTimeMillis() >= biggerBarUpTime + 10000)
					{
    		...
    		loadBricksHandler.post(new Runnable() {
				...
				public void run() {					
					...
					for (int i = 0; i < 4; i++)
					{
						...
						Log.d("Power " + Integer.toString(i), Integer.toString(power[i]));
						...
						Log.d("Power Assign " + Integer.toString(i), Integer.toString(powerAssign[i]));
					}
					    ...
					    while ((mLine = reader.readLine()) != null)
					    {
					       ...
					       if (i==6)
					       {
					    	   ...
					    	   if (powerAssign[0] == j)
					    	   {
					    		   ...
					    		   Log.d("Power & J ", Integer.toString(powerAssign[0]) + " " + Integer.toString(j));
					    	   }
					    	   else if (powerAssign[1] == j)
					    	   {
					    		   bricks.add(new Brick(left, top, right, bottom, color, brickLife, power[k++]));
					    		   Log.d("Power & J ", Integer.toString(powerAssign[1]) + " " + Integer.toString(j));
					    	   }
					    	   else if (powerAssign[2] == j)
					    	   {
					    		   bricks.add(new Brick(left, top, right, bottom, color, brickLife, power[k++]));
					    		   Log.d("Power & J ", Integer.toString(powerAssign[2]) + " " + Integer.toString(j));
					    	   }
					    	   else if (powerAssign[3] == j)
					    	   {
					    		   bricks.add(new Brick(left, top, right, bottom, color, brickLife, power[k++]));
					    		   Log.d("Power & J ", Integer.toString(powerAssign[3]) + " " + Integer.toString(j));
					    	   }
					    	   else
					    	   {
					    	   ...
					    	   Log.d("Brick " + Integer.toString(l), Integer.toString(bricks.get(l).getPower()));
					    	   i = 0;
					    	   l++;
					       }  
					    }
